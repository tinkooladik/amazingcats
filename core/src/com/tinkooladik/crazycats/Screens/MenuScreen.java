...
public class MenuScreen extends ScreenAdapter {
  ...
  private TextureActor settBtn, audio, music, sound, vibro, musicOff, leaderboard, achievements,
      rate;
  ...
  public MenuScreen(AcidCat game) {
    ...
    // achievements
    achievements = new TextureActor(
        new TextureRegion(AcidCat.manager.get(Assets.txrAchievements, Texture.class)));
    ...
    // leaderboard
    leaderboard = new TextureActor(
        new TextureRegion(AcidCat.manager.get(Assets.txrLeaderboard, Texture.class)));
    ...
    // rate
    rate = new TextureActor(new TextureRegion(AcidCat.manager.get(Assets.txrRate, Texture.class)));
    ...
    // settings
    settBtn =
        new TextureActor(new TextureRegion(AcidCat.manager.get(Assets.txrSettings, Texture.class)));
    ...
    settings = new Group();
    audio =
        new TextureActor(new TextureRegion(AcidCat.manager.get(Assets.txrAudio, Texture.class)));
    musicOff =
        new TextureActor(new TextureRegion(AcidCat.manager.get(Assets.txrMusicOff, Texture.class)));
    music = new TextureActor(
        new TextureRegion(AcidCat.manager.get(Assets.txrTranspBtn, Texture.class)));
    music.setSize(width / 8, width / 10);
    sound = new TextureActor(
        new TextureRegion(AcidCat.manager.get(Assets.txrTranspBtn, Texture.class)));
    sound.setSize(width / 8, width / 10);
    vibro = new TextureActor(
        new TextureRegion(AcidCat.manager.get(Assets.txrTranspBtn, Texture.class)));
    vibro.setSize(width / 8, width / 10);
    ...
    audio.setPosition(settBtn.getX() - audio.getWidth() - distance * 2f,
        settBtn.getY() - audio.getHeight() / 2 + settBtn.getHeight() / 2);
    settings.addActor(audio);

    music.setPosition(audio.getX() + distance, audio.getTop() - music.getHeight() - distance * 2);
    settings.addActor(music);
    sound.setPosition(audio.getX() + distance,
        audio.getTop() - sound.getHeight() * 2f - distance * 2f);
    settings.addActor(sound);
    vibro.setPosition(sound.getX(), audio.getTop() - sound.getHeight() * 3f - distance * 2f);
    settings.addActor(vibro);
  ...
  @Override public void render(float delta) {
    ...
    settBatch.begin();
    if (!Settings.musicEnabled && settShown) {
      settBatch.draw(new TextureRegion(AcidCat.manager.get(Assets.txrMusicOff, Texture.class)),
          music.getX(), music.getY(), music.getWidth(), music.getHeight());
    }
    if (!Settings.soundEnabled && settShown) {
      settBatch.draw(new TextureRegion(AcidCat.manager.get(Assets.txrMusicOff, Texture.class)),
          sound.getX(), sound.getY(), sound.getWidth(), sound.getHeight());
    }
    if (!Settings.vibroEnabled && settShown) {
      settBatch.draw(new TextureRegion(AcidCat.manager.get(Assets.txrMusicOff, Texture.class)),
          vibro.getX(), vibro.getY(), sound.getWidth(), sound.getHeight());
    }
    settBatch.end();

    if (getBonus) {
      bonusBatch.begin();
      bonusBatch.draw(new TextureRegion(AcidCat.manager.get(Assets.txrBonusBg, Texture.class)), 0,
          0, width, height);
      bonusBatch.draw(new TextureRegion(AcidCat.manager.get(Assets.txrBonusImg, Texture.class)), 20,
          (height - (width + 60)) / 2, width - 40, width + 60);
      bonusBatch.end();
    }
    ...
    if (Gdx.input.justTouched()) {
      if (getBonus) {
        ...
        Settings.save();
        getBonus = false;
      } else if (playButton.contains(Gdx.input.getX(), Gdx.input.getY())) {
